...
def verify_action_protocol_event(row, event):
    ...
    if not isinstance(event, dict) or row.get("arm") not in {"action_protocol", "combined"}:
        return False
    kind = event.get("kind")
...
async def run_trial(task, job, executor, client, *, cross_task_evidence=None, ledger_path):
    ...
    if arm not in ARMS or topology not in TOPOLOGIES:
    ...
    action_guard = arm in {"action_protocol", "combined"}
    budget = Budget()
    ...
    try:
        ...
        if action_guard and action_valid and not delivered_requests:
