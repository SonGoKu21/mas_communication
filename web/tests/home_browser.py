...
with sync_playwright() as p:
 ...
 expect(page.locator('#home-rq-3 img[src="figures/fig-7.png"]')).to_be_visible()
 expect(page.locator('[aria-label="Figure 7 case explanation"]')).to_be_visible()
 ...
 page.get_by_role('button',name='Inspect Sequential case →',exact=True).click();expect(page.locator('#detail')).to_contain_text('admin-main-sequential-41-r131-044af0c8');page.locator('[data-view=home]').click()
 page.get_by_role('button',name='Inspect Hierarchical case →',exact=True).click();expect(page.locator('#detail')).to_contain_text('hierarchical');page.locator('[data-view=home]').click()
