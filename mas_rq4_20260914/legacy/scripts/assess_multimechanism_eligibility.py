...
REQUEST_FIELDS = ('request_index', *IDENTITY_FIELDS, 'response_model', 'provider_request_id', 'request_sent',
                  'status', 'error_type', 'exception_chain', 'prompt_tokens', 'completion_tokens', 'total_tokens',
                  'prompt_cache_hit_tokens', 'prompt_cache_miss_tokens')
INFERENCE_PARAMETERS = dict(temperature=0, max_tokens=2048, disable_thinking=True,
...
def usage(row):
    ...
    for request in requests:
        ...
        require(request.get('model_version') == MODEL_VERSION, 'request_model_version_mismatch')
        status = request.get('status')
        require(status in ('success', 'error'), 'unknown_request_status')
        require(row.get('status') in ('timeout', 'infra_error') or status == 'success',
                'completed_attempt_request_error')
        unsent_guard = (row.get('status') in ('timeout', 'infra_error') and status == 'error'
            and request.get('error_type') == 'DeepSeekPeakWindowError'
        ...
        require(type(request.get('request_sent')) is bool and (request['request_sent'] or unsent_guard),
                'request_sent_mismatch')
        require('response_model' in request and (request['response_model'] == MODEL
                or request['response_model'] is None and status == 'error'), 'response_model_mismatch')
    ...
    require(log is None or isinstance(log, list) and len(log) == len(requests) and all(
        isinstance(a, dict) and all(k in a and type(a[k]) is type(v) and a[k] == v
...
def request_export(row):
    requests = row.get('model_requests')
    if not isinstance(requests, list) or any(not isinstance(r, dict) for r in requests):
        return None
    return [{k: r.get(k) for k in REQUEST_FIELDS} for r in requests]


def request_export_matches(exported, raw):
    expected = request_export(raw)
    return (expected is not None and isinstance(exported.get('requests'), list)
            and all(k in exported and type(exported[k]) is type(raw.get(k)) and exported[k] == raw[k]
                    for k in ('model_calls', 'usage_complete'))
            and canonical(exported['requests']) == canonical(expected))
...
def assess(audit_path, result_dir):
    ...
    check(config.get('model_version') == MODEL_VERSION, 'config_model_version_mismatch')
    settings = config.get('inference_settings')
    if check(isinstance(settings, dict), 'inference_settings_unknown'):
        check(settings.get('model') == MODEL and settings.get('provider') == PROVIDER, 'inference_model_mismatch')
        check(settings.get('model_version') == MODEL_VERSION, 'inference_model_version_mismatch')
        check(settings.get('api_base_url') in ('https://api.deepseek.com', 'https://api.deepseek.com/',
              'https://api.deepseek.com/v1', 'https://api.deepseek.com/v1/'), 'invalid_inference_endpoint')
        check(all(type(settings.get(k)) is type(v) and settings[k] == v
                  for k, v in INFERENCE_PARAMETERS.items()), 'inference_parameter_mismatch')
    ...
    for aid, start in sorted(start_map.items()):
        ...
        try:
            ...
            for value in (start, row):
                ...
                require(value.get('model_version') == MODEL_VERSION, 'identity_model_version_mismatch')
            ...
            if failed:
                ...
                require(isinstance(row.get('model_requests'), list) and all(
                    isinstance(r, dict) and r.get('status') == 'success' for r in row['model_requests']),
                    'completed_attempt_request_error')
            lower, exact = usage(row)
            request_details = request_export(row)
    ...
    for key, case in case_map.items():
        ...
        check(request_export_matches(case, raw), 'audit_case_requests_mismatch')
        check(all(type(case.get(k)) is type(raw.get(k)) and case.get(k) == raw.get(k)
                  for k in (*JOB_FIELDS, *IDENTITY_FIELDS, 'run_id', 'attempt_id', *OUTCOME_FIELDS))
    ...
    for aid, exported_row in exported_map.items():
        ...
        check(request_export_matches(exported_row, raw), 'audit_attempt_requests_mismatch')
        cost = attempt_costs.get(aid)
        check(all(exported_row.get(k) == raw.get(k) for k in (*JOB_FIELDS, *IDENTITY_FIELDS, 'run_id', 'attempt'))
    ...
    return dict(version=VERSION, policy_version=POLICY_VERSION, offline=True,
                **{k: config.get(k) for k in IDENTITY_FIELDS}, model_identity_immutable=False,
                ...
                limitations=['model_calls counts client request attempts, not paid API calls; request_sent records network dispatch only.',
                             'The API name and documented model version are not immutable model-weight evidence.',
                             'Eligibility trusts the hash-bound offline audit for source, semantic, exposure and trace validation; '
