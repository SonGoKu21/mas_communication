...
MODEL_IDENTITY = {"model": "deepseek-flash", "provider": "deepseek", "model_version": "DeepSeek-V4.1-Flash"}
REQUEST_FIELDS = ("request_index", *MODEL_IDENTITY, "response_model", "provider_request_id", "request_sent", "status",
...
LIMITATIONS = [
    "model_calls 是客户端尝试数，不是实际付费 API 调用数；request_sent=False 仅允许有零 usage 证据的发送前忙时拒绝。",
    "未提供 result-dir 时仅检查请求导出的结构和相互一致性，不宣称已核验 raw 请求或真实付费调用。",
    "模型 API 名和文档版本声明不是 immutable 权重证据；本报告仅接受本轮 V4.1-Flash 身份。",
...
def validate_request_export(row, *, failed=False):
    if failed:
        require(row.get('status') in ('timeout', 'infra_error'), 'request attempt failure status mismatch')
    requests = row.get("requests")
    require(isinstance(requests, list), "request exports required")
    require(type(row.get("model_calls")) is int and row["model_calls"] == len(requests),
            "request export count mismatch")
    indices = []
    for request in requests:
        require(isinstance(request, dict) and set(request) == set(REQUEST_FIELDS), "request export schema mismatch")
        require(all(request[k] == v for k, v in MODEL_IDENTITY.items()), "request model identity mismatch")
        index = request["request_index"]
        require(type(index) is int and index > 0, "request index invalid")
        indices.append(index)
        status = request["status"]
        require(status in ("success", "error") and (failed or status == "success"),
                "completed attempt request status mismatch")
        unsent_guard = (failed and row.get("status") in ("timeout", "infra_error") and status == "error"
            and request["error_type"] == "DeepSeekPeakWindowError"
            and request["exception_chain"] == ["DeepSeekPeakWindowError"]
            and request["response_model"] is None and request["provider_request_id"] == ""
            ...
            and (request["total_tokens"] is None or type(request["total_tokens"]) is int and request["total_tokens"] == 0))
        require(type(request["request_sent"]) is bool and (request["request_sent"] or unsent_guard),
                "request sent mismatch")
        require(request["response_model"] == "deepseek-flash" or
                failed and status == "error" and request["response_model"] is None, "request response model mismatch")
        chain = request["exception_chain"]
        require(chain is None or isinstance(chain, list) and all(isinstance(x, str) and x for x in chain),
                "request exception chain invalid")
        if status == "error":
            require(isinstance(request["error_type"], str) and request["error_type"] and chain
                    and chain[0] == request["error_type"], "request error identity missing")
        else:
            require(not chain and not request["error_type"], "request status error conflict")
            require(isinstance(request["provider_request_id"], str) and request["provider_request_id"],
                    "request ID missing")
        for field in ("prompt_tokens", "completion_tokens", "total_tokens", "prompt_cache_hit_tokens", "prompt_cache_miss_tokens"):
            value = request[field]
            require(value is None or type(value) is int and value >= 0, "request usage invalid")
        prompt, completion, total = (request[k] for k in ("prompt_tokens", "completion_tokens", "total_tokens"))
        require(status == "error" or prompt is not None and completion is not None, "request usage unknown")
        if prompt is not None and completion is not None:
            require(total is None or total == prompt + completion, "request usage mismatch")
    if indices:
        require(indices == list(range(indices[0], indices[0] + len(indices))), "request index sequence mismatch")
    return requests
...
def validate_inputs(audit, manifest):
    ...
    require(all(config.get(k) == v for k, v in MODEL_IDENTITY.items()), "config model identity mismatch")
    settings = config.get("inference_settings")
    require(isinstance(settings, dict), "inference settings required")
    require(all(settings.get(k) == v for k, v in MODEL_IDENTITY.items()), "inference model identity mismatch")
    require(settings.get("api_base_url") in ("https://api.deepseek.com", "https://api.deepseek.com/",
            "https://api.deepseek.com/v1", "https://api.deepseek.com/v1/"), "invalid inference endpoint")
    require(all(type(settings.get(k)) is type(v) and settings[k] == v
                for k, v in INFERENCE_PARAMETERS.items()), "inference parameter mismatch")
    ...
    attempts = audit.get("attempt_cases")
    require(isinstance(attempts, list), "request attempt exports required")
    attempt_map = {}
    for attempt in attempts:
        require(isinstance(attempt, dict), "request attempt export must be an object")
        aid = attempt.get("attempt_id")
        require(isinstance(aid, str) and aid and aid not in attempt_map, "duplicate or missing request attempt ID")
        kind = attempt.get("terminal_kind")
        require(kind in ("completed", "error"), "request attempt terminal kind invalid")
        validate_request_export(attempt, failed=kind == "error")
        attempt_map[aid] = attempt
    indexed, seen = {}, {k: set() for k in ("run_id", "job_key", "attempt_id")}
    for case in cases:
        key = key_of(case)
        require(all(case.get(k) == v for k, v in MODEL_IDENTITY.items()), "case model identity mismatch")
        ...
        for field, values in seen.items():
            ...
            values.add(value)
        requests = validate_request_export(case)
        attempt = attempt_map.get(case["attempt_id"])
        require(attempt is not None and attempt["terminal_kind"] == "completed"
                and all(attempt.get(k) == case.get(k) for k in (*KEY, "arm", "job_key", "pair_key", "run_id"))
                and canonical(attempt["requests"]) == canonical(requests), "case/attempt request exports mismatch")
...
def summarize(audit_path, manifest_path, *, draws=10000, seed=20260911, result_dir=None):
    ...
    report = {"version": "shopping-multimechanism-formal-stats-v1", "offline": True,
        **{k: config[k] for k in MODEL_IDENTITY}, "model_identity_immutable": False,
