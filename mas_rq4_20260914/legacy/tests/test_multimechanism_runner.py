...
def offpeak_clock(monkeypatch):
    from mas_faults import deepseek_schedule as schedule
    class Weekend(datetime):
        @classmethod
        def now(cls, tz=None):
            return datetime(2026, 9, 12, 10, tzinfo=schedule.BEIJING).astimezone(tz)
    monkeypatch.setattr(schedule, "datetime", Weekend)
...
def local_env():
    return {"LLM_PROVIDER": "deepseek", "LLM_MODEL": "deepseek-flash",
            "LLM_BASE_URL": "https://api.deepseek.com", "LLM_API_KEY": "test-secret",
...
@pytest.mark.parametrize("change", [{"LLM_PROVIDER": "modelscope_local"}, {"LLM_MODEL": "deepseek-chat"},
    {"LLM_BASE_URL": "http://127.0.0.1:18001/v1"}, {"LLM_PROVIDER": ""},
...
class OfflineClient:
    ...
    def __init__(self):
        self.model_info = SimpleNamespace(provider="deepseek", model="deepseek-flash",
            base_url="https://api.deepseek.com", client_type="http_openai_compatible")
    ...
    def record(self):
        ...
        self.request_log.append({"request_index": self.call_count, "prompt_tokens": 5,
                                 ...
                                 "status": "success", "provider": "deepseek", "model": "deepseek-flash",
...
def test_completed_nullable_usage_persists_and_reports_without_retries(tmp_path):
    ...
    async def trial(task, job, executor, received_client, **kwargs):
        ...
        received_client.request_log.append({"request_index": 1, "prompt_tokens": None,
                                            ...
                                            "response_model": "deepseek-flash", "request_sent": True})
...
def test_preflight_checks_real_model_listing_without_completion_calls(monkeypatch):
    ...
    def models(request, *, timeout):
        ...
        assert timeout == 10
        return io.BytesIO(json.dumps({"data": [{"id": "deepseek-flash"}, {"id": "deepseek-v4-pro"}]}).encode())
...
def test_preflight_rejects_changed_env_before_client_creation(monkeypatch):
    ...
    monkeypatch.setenv("LLM_PROVIDER", "modelscope_local")
