...
from test_multimechanism_runner import local_env, offpeak_clock
...
def test_exact_18_cells_freeze_original_input_then_select(setup, monkeypatch):
    ...
    assert config["model"] == "deepseek-flash" and config["provider"] == "deepseek"
    assert config["model_version"] == "DeepSeek-V4.1-Flash"
    settings = config["inference_settings"]
    assert settings["model"] == "deepseek-flash" and settings["provider"] == "deepseek"
    assert settings["model_version"] == "DeepSeek-V4.1-Flash"
    assert settings["api_base_url"] == "https://api.deepseek.com/v1"
    assert settings["disable_thinking"] is True and settings["max_tokens"] == 2048
    assert (settings["socket_timeout_seconds"], settings["total_timeout_seconds"]) == (90, 120)
...
@pytest.mark.parametrize("change", [{"LLM_PROVIDER": "modelscope_local"}, {"LLM_MODEL": "other"},
...
def test_flash_peak_startup_is_auditable_without_preflight_or_attempt(setup, monkeypatch):
    m = launcher()
    args, _ = setup
    monkeypatch.setattr(m, "sys", SimpleNamespace(platform="linux"))
    class PeakClock(datetime):
        @classmethod
        def now(cls, tz=None):
            return datetime(2026, 9, 14, 9, tzinfo=schedule.BEIJING).astimezone(tz)
    monkeypatch.setattr(schedule, "datetime", PeakClock)
    monkeypatch.setattr(m.runner, "preflight_client", lambda *_: pytest.fail("busy startup reached preflight"))
    monkeypatch.setattr(m.runner, "execute_jobs", lambda *a, **k: pytest.fail("busy startup reached jobs"))
    result = m.run_regression(args)
    assert result["regression_passed"] is False and result["completed_runs"] == 0
    assert result["error_attempts"] == 0
    assert m.runner.read_jsonl(args.output_dir / "run_attempts.jsonl") == []
    ...
    assert list((args.output_dir / "scheduler_stops").glob("*.json"))
