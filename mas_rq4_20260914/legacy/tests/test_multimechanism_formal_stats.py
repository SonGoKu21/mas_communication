...
def fixture(products=("a", "a", "b"), repetitions=2):
        ...
        for repeat in range(1, repetitions + 1):
            for condition, boundary in (("clean", None), ("request_non_delivery", "action_request")):
                ...
                for arm in ARMS:
                    ...
                    cases[-1].update(model_calls=2, usage_complete=True)
                    cost = cases[-1]['total_tokens']
                    cases[-1]['requests'] = [{
                        "request_index": i, "model": "deepseek-flash", "provider": "deepseek",
                        "model_version": "DeepSeek-V4.1-Flash", "response_model": "deepseek-flash",
                        "provider_request_id": f"response-{len(cases)}-{i}", "request_sent": True,
                        "status": "success", "error_type": None, "exception_chain": None,
                        "prompt_tokens": cost // 2 - 2, "completion_tokens": 2, "total_tokens": cost // 2,
    ...
    config = {"tasks": tasks, "jobs": jobs, "repetitions": repetitions, "planned_runs": len(jobs),
              ...
              "model": "deepseek-flash", "provider": "deepseek", "model_version": "DeepSeek-V4.1-Flash",
              "inference_settings": {"model": "deepseek-flash", "provider": "deepseek",
    ...
    audit = {"audit_version": "shopping-multimechanism-offline-v1", "offline": True,
        ...
        "attempt_cases": [dict(copy.deepcopy(c), terminal_kind="completed") for c in cases],
...
@pytest.mark.parametrize("scope", ["config", "settings", "case"])
...
def test_formal_summary_rejects_mixed_or_legacy_identity(tmp_path, scope, field, value):
    audit, manifest = fixture()
    target = {"config": manifest["config"], "settings": manifest["config"]["inference_settings"],
              "case": audit["cases"][0]}[scope]
    target[field] = value
    manifest["config_digest"] = digest(manifest["config"])
    with pytest.raises(ValueError, match="model.*mismatch"):
        module().summarize(*inputs(tmp_path, audit, manifest), draws=1)


@pytest.mark.parametrize("field,value", [("api_base_url", "http://127.0.0.1:18001/v1"),
                                         ("max_tokens", 64), ("disable_thinking", False)])
def test_formal_summary_rejects_wrong_inference_settings(tmp_path, field, value):
    audit, manifest = fixture()
...
def test_formal_summary_labels_api_identity_without_immutable_weight_claim(tmp_path):
    report = module().summarize(*inputs(tmp_path), draws=1)
    assert {k: report[k] for k in ("model", "provider", "model_version")} == {
        "model": "deepseek-flash", "provider": "deepseek", "model_version": "DeepSeek-V4.1-Flash"}
    assert report["model_identity_immutable"] is False


@pytest.mark.parametrize("scope", ["cases", "attempt_cases", "both"])
@pytest.mark.parametrize("damage", ["model", "provider", "version", "response", "request_id",
                                    "sent", "index", "count", "usage", "missing"])
def test_legacy_cli_rejects_request_export_contradictions(tmp_path, scope, damage):
    audit, manifest = fixture()
    targets = [audit[k][0] for k in (["cases", "attempt_cases"] if scope == "both" else [scope])]
    for target in targets:
        ...
        if damage == 'count':
            ...
            field, value = {
                'model': ('model', 'deepseek-v4-flash'), 'provider': ('provider', 'other'),
                'version': ('model_version', 'DeepSeek-V4-Flash'),
                'response': ('response_model', 'deepseek-v4-flash'),
                'request_id': ('provider_request_id', '' if scope == 'both' else 'different-id'),
                'sent': ('request_sent', False), 'index': ('request_index', True),
...
@pytest.mark.parametrize('damage', ['case_model', 'attempt_model', 'coherent_request_id', 'hidden_error'])
def test_bound_formal_rejects_raw_export_request_contradictions(tmp_path, damage):
    from test_multimechanism_eligibility import fixture as eligible_fixture, save, export_requests
    audit, manifest, logs = values = eligible_fixture(formal=True)
    if damage == 'hidden_error':
        ...
        for target in (audit['cases'][0], audit['attempt_cases'][0]):
            target['requests'] = export_requests(row)
    elif damage == 'coherent_request_id':
        for target in (audit['cases'][0], audit['attempt_cases'][0]):
            target['requests'][0]['provider_request_id'] = 'coherent-but-not-raw'
    else:
        audit['cases' if damage == 'case_model' else 'attempt_cases'][0]['requests'][0]['model'] = 'deepseek-v4-flash'
    refresh(audit)
    ap, root = save(tmp_path, values)
    try:
        report = module().summarize(ap, root / 'matrix_manifest.json', result_dir=root, draws=1)
    except ValueError as exc:
        assert 'request' in str(exc)
    else:
        assert not report['formal_eligible'] and not report['exact_cost_comparison_eligible']
        assert report['summaries'] == []


def test_legacy_consistent_exports_do_not_claim_raw_verification(tmp_path):
    audit, manifest = fixture()
    for target in (audit['cases'][0], audit['attempt_cases'][0]):
        target['requests'][0]['provider_request_id'] = 'coherent-export-only-id'
    report = module().summarize(*inputs(tmp_path, audit, manifest), draws=1)
    assert report['status'] == 'complete'
    assert report['eligibility'] is None and not report['formal_eligible']
...
def test_full_frozen_6300_without_result_binding_is_descriptive_not_formal(tmp_path):
    ...
    audit['attempt_cases'] = [dict(copy.deepcopy(c), terminal_kind='completed') for c in audit['cases']]
...
def test_real_offline_auditor_request_exports_fit_legacy_summary(tmp_path):
    from test_multimechanism_audit import snapshot, completed, write_rows, audit as run_audit
    root, config, digest = snapshot(tmp_path)
    row = completed(config['jobs'][0], digest)
    write_rows(root, [row])
    source_audit = run_audit(root)
    path = tmp_path / 'actual-audit.json'
    path.write_text(json.dumps(source_audit))
    report = module().summarize(path, root / 'matrix_manifest.json', draws=1)
    assert report['eligibility'] is None and not report['formal_eligible']
    assert not report['exact_cost_comparison_eligible']
    assert report['source_audit_status'] == source_audit['status']


@pytest.mark.parametrize('damage', [None, 'wrong_error', 'usage', 'sent_type', 'response', 'completed_attempt'])
def test_legacy_unsent_failed_export_requires_offpeak_zero_usage(tmp_path, damage):
    from test_multimechanism_eligibility import offpeak_rejection_fixture
    audit, manifest, _ = offpeak_rejection_fixture()
    request = audit['attempt_cases'][-1]['requests'][1]
    if damage == 'wrong_error':
        request.update(error_type='TimeoutError', exception_chain=['TimeoutError'])
    elif damage == 'usage':
        request['prompt_cache_hit_tokens'] = 1
    elif damage == 'sent_type':
        request['request_sent'] = 0
    elif damage == 'response':
        request['response_model'] = 'deepseek-flash'
    elif damage == 'completed_attempt':
        audit['attempt_cases'][-1]['status'] = 'completed'
        request['request_sent'] = True
    refresh(audit)
    paths = inputs(tmp_path, audit, manifest)
    if damage:
        with pytest.raises(ValueError, match='request'):
            module().summarize(*paths, draws=1)
    else:
        report = module().summarize(*paths, draws=1)
        assert report['eligibility'] is None and not report['formal_eligible']
        assert not report['exact_cost_comparison_eligible']
