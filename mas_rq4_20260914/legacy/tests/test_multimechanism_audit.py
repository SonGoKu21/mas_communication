...
def completed(job, digest, t=None, index=1):
    ...
    request = {"request_index": index, "model": "deepseek-flash", "provider": "deepseek",
               "model_version": "DeepSeek-V4.1-Flash",
               "status": "success", "response_model": "deepseek-flash", "provider_request_id": f"response-{index}",
               "request_sent": True,
...
def snapshot(tmp_path, *, tasks=None):
    ...
    config = {"version": matrix.VERSION, "runner_schema": 1, "tasks": ts, "jobs": jobs,
              ...
              "source_hashes": {}, "inference_settings": {
                  "model": "deepseek-flash", "provider": "deepseek", "model_version": "DeepSeek-V4.1-Flash",
                  "api_base_url": "https://api.deepseek.com/v1", "temperature": 0,
...
def write_rows(root, rows, errors=(), starts=None):
    if starts is None:
        starts = [{k: v for k, v in r.items() if k in {
            *matrix.build_jobs([task()], 1)[0], "attempt", "attempt_id", "config_digest", "model", "provider", "model_version",
...
@pytest.mark.parametrize("scope", ["config", "settings", "start", "row", "request"])
@pytest.mark.parametrize("field,value,code", [
    ("model", "Qwen/Qwen3.8-27B", "model_mismatch"),
    ("model", "deepseek-v4-flash", "model_mismatch"),
    ("provider", "modelscope_local", "provider_mismatch"),
    ("model_version", "DeepSeek-V4-Flash", "model_version_mismatch"),
    ("model_version", None, "model_version_mismatch"),
])
def test_flash_identity_rejected_at_every_audit_boundary(tmp_path, scope, field, value, code):
    root, config, digest = snapshot(tmp_path)
    row = completed(config["jobs"][0], digest)
    start = copy.deepcopy(row)
    target = {"config": config, "settings": config["inference_settings"],
              "start": start, "row": row, "request": row["model_requests"][0]}[scope]
    target[field] = value
    if scope in {"config", "settings"}:
        digest = matrix.config_digest(config)
        row["config_digest"] = start["config_digest"] = digest
        (root / "matrix_manifest.json").write_text(json.dumps({"config": config, "config_digest": digest}))
    write_rows(root, [row], starts=[start])
    result = audit(root)
...
def test_official_flash_settings_accepted_and_identity_preserved(tmp_path, endpoint):
    root, config, _ = snapshot(tmp_path)
    config["inference_settings"]["api_base_url"] = endpoint
    digest = matrix.config_digest(config)
    (root / "matrix_manifest.json").write_text(json.dumps({"config": config, "config_digest": digest}))
    auditor = auditor_module().Auditor(root)
    auditor.manifest()
    assert auditor.findings == []
    row = completed(config["jobs"][0], digest)
    write_rows(root, [row])
    result = audit(root)
    for exported in (result, result["cases"][0], result["attempt_cases"][0]):
        assert {k: exported[k] for k in ("model", "provider", "model_version")} == {
            "model": "deepseek-flash", "provider": "deepseek", "model_version": "DeepSeek-V4.1-Flash"}
    assert result["model_identity_immutable"] is False
...
@pytest.mark.parametrize("field,value,code", [
    ...
    ("api_base_url", "https://user@api.deepseek.com/v1", "invalid_inference_endpoint"),
    ("api_base_url", "https://api.deepseek.com:8443/v1", "invalid_inference_endpoint"),
    ("api_base_url", "https://api.deepseek.com/v1?route=old", "invalid_inference_endpoint"),
    ("api_base_url", "https://api.deepseek.com/v1#old", "invalid_inference_endpoint"),
    ("api_base_url", "https://api.deepseek.com/other", "invalid_inference_endpoint"),
    ("api_base_url", None, "invalid_inference_endpoint"),
    ("temperature", True, "inference_parameter_mismatch"),
    ("temperature", 0.5, "inference_parameter_mismatch"),
...
def test_flash_inference_settings_fail_closed(tmp_path, field, value, code):
    root, config, _ = snapshot(tmp_path)
    config["inference_settings"][field] = value
    digest = matrix.config_digest(config)
    (root / "matrix_manifest.json").write_text(json.dumps({"config": config, "config_digest": digest}))
    auditor = auditor_module().Auditor(root)
    auditor.manifest()
    assert code in {f["code"] for f in auditor.findings}


@pytest.mark.parametrize("status,response_model", [
    ("success", None), ("success", "DeepSeek-V4.1-Flash"), ("success", "deepseek-v4-flash"),
    ("error", "Qwen/Qwen3.8-27B"), (None, "deepseek-flash"),
])
def test_flash_response_model_is_not_an_open_alias_allowlist(tmp_path, status, response_model):
    root, config, digest = snapshot(tmp_path)
    row = completed(config["jobs"][0], digest)
    row["model_requests"][0].update(status=status, response_model=response_model)
    write_rows(root, [row])
    assert "response_model_mismatch" in codes(audit(root))


def test_flash_raw_response_identity_preserved_and_error_none_allowed(tmp_path):
    root, config, digest = snapshot(tmp_path)
    row = completed(config["jobs"][0], digest)
    write_rows(root, [row])
    result = audit(root)
    request = result["attempt_cases"][0]["requests"][0]
    assert request["response_model"] == "deepseek-flash" and request["provider_request_id"] == "response-1"
    row["model_requests"][0].update(status="error", response_model=None,
                                    error_type="TimeoutError", exception_chain=["TimeoutError"])
    row.update(status="timeout", error_type="TimeoutError", usage_complete=False)
    write_rows(root, [], [row])
    assert "response_model_mismatch" not in codes(audit(root))


@pytest.mark.parametrize("status", ["completed", "unresolved", "timeout"])
@pytest.mark.parametrize("response_model", [None, "deepseek-flash"])
def test_completed_attempt_cannot_hide_error_request_with_known_usage(tmp_path, status, response_model):
    root, config, digest = snapshot(tmp_path)
    row = completed(config["jobs"][0], digest)
    row["status"] = status
    ...
    write_rows(root, [row])
    result = audit(root)
    assert "completed_attempt_request_error" in codes(result)
    assert result["status"] == "failed"


@pytest.mark.parametrize("damage", ["missing_response", "unproven_error", "log_response", "log_version", "log_request_id"])
def test_error_response_none_requires_bound_partial_request_record(tmp_path, damage):
    root, config, digest = snapshot(tmp_path)
    row = completed(config["jobs"][0], digest)
    first = row["model_requests"][0]
    last = {**first, "request_index": 2, "status": "error", "response_model": None,
            "provider_request_id": "", "prompt_tokens": None, "completion_tokens": None,
            "error_type": "TimeoutError", "exception_chain": ["TimeoutError"]}
    row.update(status="timeout", error_type="TimeoutError", usage_complete=False, model_calls=2,
               model_requests=[first, last], llm_request_log=copy.deepcopy([first, last]))
    expected = "request_logs_mismatch"
    if damage == "missing_response":
        last.pop("response_model")
        expected = "response_model_mismatch"
    elif damage == "unproven_error":
        last.pop("error_type")
        last.pop("exception_chain")
    ...
    assert result["tokens"]["error_attempts_known_tokens"] == 15
...
def test_unsent_offpeak_rejection_has_narrow_zero_usage_evidence(tmp_path, damage):
    root, config, digest = snapshot(tmp_path)
    row = completed(config['jobs'][0], digest)
    request = row['model_requests'][0]
    request.update(status='error', response_model=None, provider_request_id='', request_sent=False,
                   error_type='DeepSeekPeakWindowError', exception_chain=['DeepSeekPeakWindowError'],
                   prompt_tokens=0, completion_tokens=0, total_tokens=0,
                   prompt_cache_hit_tokens=0, prompt_cache_miss_tokens=0)
    row.update(status='infra_error', error_type='DeepSeekPeakWindowError', usage_complete=False,
    ...
    if damage == 'success':
        ...
        request['prompt_cache_hit_tokens'] = None
        ...
        row['llm_request_log'][0]['request_sent'] = True
    elif damage == 'log_sent_numeric':
        row['llm_request_log'][0]['request_sent'] = 0
    write_rows(root, [], [row])
    result = audit(root)
    if damage:
        assert ('request_logs_mismatch' if damage.startswith('log_sent') else 'request_sent_mismatch') in codes(result)
    else:
        assert 'request_sent_mismatch' not in codes(result)
        exported = result['attempt_cases'][0]
        assert exported['model_calls'] == 1 and exported['requests'][0]['request_sent'] is False
        assert exported['known_total_tokens'] == 0
...
def test_inference_config_is_checked_and_failed_attempt_costs_are_separate(tmp_path):
    ...
    result = audit(root)
    assert "invalid_inference_endpoint" in codes(result)
    assert result["tokens"]["total_tokens"] is None
