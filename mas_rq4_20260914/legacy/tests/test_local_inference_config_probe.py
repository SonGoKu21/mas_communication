...
def test_local_service_probe_identity_is_isolated_from_flash_runner(probe, monkeypatch, flash):
    from test_multimechanism_runner import local_env
    env = local_env() if flash else {"LLM_PROVIDER": "modelscope_local",
        "LLM_MODEL": "Qwen/Qwen3.8-27B", "LLM_BASE_URL": "http://127.0.0.1:18001/v1",
        "LLM_API_KEY": "unit-secret", "LLM_MAX_TOKENS": "128", "LLM_DISABLE_THINKING": "1",
        "LLM_REQUEST_TIMEOUT_SECONDS": "2", "LLM_TOTAL_REQUEST_TIMEOUT_SECONDS": "2"}
    ...
    if flash:
        ...
        assert settings["model"] == "Qwen/Qwen3.8-27B"
        assert settings["provider"] == "modelscope_local"
        assert settings["max_tokens"] == 128
    assert probe.runner.MODEL == "deepseek-flash" and probe.runner.PROVIDER == "deepseek"


@pytest.mark.parametrize("served_model", ["Qwen/Qwen3.8-27B", "deepseek-flash"])
def test_local_preflight_never_uses_flash_preflight(probe, settings, monkeypatch, served_model):
    before = (probe.runner.MODEL, probe.runner.PROVIDER)
    monkeypatch.setattr(probe.runner, "preflight_client", lambda *_: pytest.fail("local probe called Flash preflight"))
    requests = []
    def listing(request, *, timeout):
        requests.append(request)
        assert request.full_url == "http://127.0.0.1:18001/v1/models"
        assert request.get_method() == "GET" and timeout == 10
        assert request.get_header("Authorization") == "Bearer unit-secret"
        return io.BytesIO(json.dumps({"data": [{"id": served_model}]}).encode())
    monkeypatch.setattr(urllib.request, "urlopen", listing)
    if served_model != "Qwen/Qwen3.8-27B":
        with pytest.raises(ValueError, match="model"):
            probe.preflight_client(settings)
    else:
        client, receipt = probe.preflight_client(settings)
        assert client.model_info.model == "Qwen/Qwen3.8-27B" and client.call_count == 0
        assert receipt["model_calls"] == 0
    assert len(requests) == 1 and before == (probe.runner.MODEL, probe.runner.PROVIDER)


def test_local_probe_does_not_relax_flash_guards_in_same_process(probe, monkeypatch):
    from datetime import datetime
    from mas_faults.deepseek_schedule import BEIJING, DeepSeekPeakWindowError, ensure_deepseek_offpeak
    from test_multimechanism_runner import local_env
    flash = local_env()
    def verify_flash():
        assert probe.runner.inference_settings(flash)["model"] == "deepseek-flash"
        for changes in ({"LLM_PROVIDER": "modelscope_local", "LLM_MODEL": "Qwen/Qwen3.8-27B"},
                        {"LLM_BASE_URL": "http://127.0.0.1:18001/v1"},
                        {"LLM_BASE_URL": "https://other.invalid"}):
            with pytest.raises(ValueError):
                probe.runner.inference_settings({**flash, **changes})
        with pytest.raises(DeepSeekPeakWindowError):
            ensure_deepseek_offpeak("deepseek-flash", now=datetime(2026, 9, 14, 10, tzinfo=BEIJING), enabled=False)
    verify_flash()
    with monkeypatch.context() as local:
        for key, value in {**flash, "LLM_PROVIDER": "modelscope_local", "LLM_MODEL": "Qwen/Qwen3.8-27B",
            "LLM_BASE_URL": "http://127.0.0.1:18001/v1", "LLM_MAX_TOKENS": "128"}.items():
            local.setenv(key, value)
        assert probe.checked_settings()["model"] == "Qwen/Qwen3.8-27B"
    verify_flash()
...
def test_crossover_exact_requests_warmup_excluded_cost_included(probe, settings, tmp_path):
    ...
    for round_number in (1, 2):
        ...
        assert sorted(by_mode["serial"], key=lambda case: case["case_id"]) == sorted(
            by_mode["parallel"], key=lambda case: case["case_id"])
