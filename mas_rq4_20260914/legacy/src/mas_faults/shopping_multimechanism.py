...
def verify_action_protocol_event(row, event):
    ...
    if not isinstance(event, dict) or row.get("arm") not in {"action_protocol", "combined", "action_always"}:
        return False
    kind = event.get("kind")
...
async def run_trial(task, job, executor, client, *, cross_task_evidence=None, ledger_path):
    ...
    if (arm not in ARMS and not (job.get("diagnostic") and arm == "action_always")) or topology not in TOPOLOGIES:
        raise ValueError("unsupported strategy or topology")
    session = uuid.uuid4().hex
    run_id = "mm-" + session
    dep = arm in {"dependency", "combined"}
    independent = arm in {"independent", "combined"}
    action_guard = arm in {"action_protocol", "combined", "action_always"}
    budget = Budget(max_replays=2) if job.get("diagnostic") else Budget()
    ...
    def readback(kind, before, *, common=False):
        ...
        recoveries.append({"kind": kind, "common": common, "receipt_indices": indices,
                           ...
                           "verified": bool(indices and check_evidence(task, p, require_success=False).valid),
    ...
    try:
        ...
        fault = SingleBoundaryFault(job["condition"], old_evidence=initial_observation,
                                    old_judgment=old_judgment, cross_task_evidence=cross_task_evidence)
        if job.get("diagnostic"):
            fault = executor.diagnostic_exposure
            executor.activate_diagnostic()
        ...
        replay_limit = 2 if job.get("diagnostic") else 1
        replay_count = 0
        while action_guard and action_valid and not delivered_requests and replay_count < replay_limit:
            ...
            if state["state"] == "pending" and state["execution_count"] == 0:
                ...
                delivered_requests = fault.deliver("action_request", request) if job.get("diagnostic") else [request]
            else:
                break
        acknowledgments = []
        ...
        if topology == "flat":
            ...
            direct = envelope(executor.reobserve_cart(task), "direct-peer", 4, current_action_id, source="EvidencePeer")
            ...
            final_e = {**direct, "payload": peer.get("payload", {})}
