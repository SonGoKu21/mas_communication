"""Server-side DeepSeek V4.1-Flash Shopping matrix; no mock mode or transfers.

Set LLM_PROVIDER=deepseek, LLM_MODEL=deepseek-flash,
LLM_BASE_URL=https://api.deepseek.com, and LLM_API_KEY explicitly.
Paid requests run only during Beijing off-peak hours, including all weekends.
...
from mas_faults.llm_client import DEEPSEEK_MODEL_VERSION, get_llm_client, openai_api_base, validate_deepseek_url
from mas_faults.llm_client import _NoRedirect, deepseek_transport as local_inference_transport
...
PROVIDER = "deepseek"
MODEL_VERSION = DEEPSEEK_MODEL_VERSION
MAX_ATTEMPTS = 2
...
REQUEST_FIELDS = ("request_index", "provider_request_id", "provider", "model", "model_version",
                  "response_model", "request_sent", "json_mode",
...
class ModelRequestAuditError(RuntimeError):
    """A returned trial cannot complete with failed or unverified model requests."""
...
def inference_settings(environ=None):
    ...
    # Paid runs must freeze explicit identity, endpoint and credentials.
    if env.get("LLM_PROVIDER") != PROVIDER or env.get("LLM_MODEL") != MODEL:
        raise ValueError("explicit provider=deepseek and model=deepseek-flash required")
    base = validate_deepseek_url(env.get("LLM_BASE_URL", ""))
    if not env.get("LLM_API_KEY", "").strip():
        raise ValueError("explicit LLM_API_KEY required")
    socket_timeout = int(env.get("LLM_REQUEST_TIMEOUT_SECONDS", "90"))
    total_timeout = int(env.get("LLM_TOTAL_REQUEST_TIMEOUT_SECONDS", "120"))
    max_tokens = int(env.get("LLM_MAX_TOKENS", "2048"))
    disable_thinking = env.get("LLM_DISABLE_THINKING", "true").lower() in {"1", "true", "yes"}
    if (socket_timeout, total_timeout, max_tokens) != (90, 120, 2048) or not disable_thinking:
        raise ValueError("frozen non-thinking 2048-token, 90s/120s budget required")
    return {"provider": PROVIDER, "model": MODEL, "model_version": MODEL_VERSION,
            "api_base_url": openai_api_base(base),
...
def build_config(args, tasks):
    ...
    if getattr(args, "diagnostic", False):
        from mas_faults.recovery_diagnostic import build_jobs
        if args.repetitions != 1 or args.shard_count != 1:
            raise ValueError("diagnostic requires one repeat and serial execution")
        all_jobs = build_jobs(tasks)
    selected = matrix.select_shard(all_jobs, args.shard_index, args.shard_count)
    return {"version": "recovery-diagnostic-v1" if getattr(args, "diagnostic", False) else matrix.VERSION, "runner_schema": 1, "tasks": copy.deepcopy(tasks),
...
@contextmanager
def _locked_file(path, *, wait=False):
...
@contextmanager
def locked_workflow(path=None, *, wait=False):
    # One host-wide lock also covers localhost/IPv4/IPv6 aliases and new outputs.
    with _locked_file(Path(path) if path is not None else Path("/tmp/mas-shopping-multimechanism.lock"), wait=wait):
        yield
...
def completed_usage(client, before, result):
    usage = usage_since(client, before, complete=True)
    requests = usage["model_requests"]
    peak = any(r.get("error_type") == "DeepSeekPeakWindowError"
               or "DeepSeekPeakWindowError" in (r.get("exception_chain") or []) for r in requests)
    invalid = len(requests) != usage["model_calls"] or any(
        r.get("status") != "success" or r.get("provider") != PROVIDER
        or r.get("model") != MODEL or r.get("model_version") != MODEL_VERSION
        or r.get("response_model") != MODEL or r.get("request_sent") is not True for r in requests)
    if peak or invalid:
        error = (DeepSeekPeakWindowError("trial swallowed an off-peak refusal") if peak else
                 ModelRequestAuditError("trial returned with failed or unverified model requests"))
        error.partial_trial = result
        raise error
    return usage
...
def _exception_is(exc, expected):
...
def is_timeout_exception(exc):
    return _exception_is(exc, TimeoutError)


def is_peak_exception(exc):
    return _exception_is(exc, DeepSeekPeakWindowError)


def record_peak_stop(output, digest, *, phase, start=None):
    stop = {"kind": "scheduler_stop", "error_type": "DeepSeekPeakWindowError",
            "config_digest": digest, "phase": phase, "timestamp_unix": time.time(),
            "resume_policy": "offpeak_only_remaining_attempt_budget"}
    if start is not None:
        stop.update({key: start[key] for key in ("job_key", "attempt_id", "attempt", "lane") if key in start})
    directory = output / "scheduler_stops"
    directory.mkdir(exist_ok=True)
    _sync_directory(output)
    _write_json(directory / (uuid.uuid4().hex + ".json"), stop)
    return stop


def guard_paid_work(output, digest, *, phase):
    try:
        ensure_deepseek_offpeak(MODEL)
    except DeepSeekPeakWindowError:
        return record_peak_stop(output, digest, phase=phase)
    return None
...
def write_reports(output, jobs, state, source_blocked=(), *, scheduler_stop=None):
    rows, errors = state["rows"], state["errors"]
    diagnostic = bool(jobs and jobs[0].get("diagnostic"))
    arms = ("baseline", "action_always", "combined") if diagnostic else matrix.ARMS
    ...
    summary = {"version": "recovery-diagnostic-v1" if diagnostic else matrix.VERSION, "status": "complete" if len(done) == len(jobs) else "incomplete",
               ...
               "by_arm": {}}
    if diagnostic:
        summary['complete_three_arm_pairs'] = sum(set(arms).issubset(p) for p in pairs.values())
        summary.pop('complete_seven_arm_pairs')
        summary['actual_exposure_counts'] = dict(Counter(len(r.get('fault_events', [])) for r in rows))
        summary['outcome_classes'] = dict(Counter(r.get('outcome_class', 'unknown') for r in rows))
    for arm in arms:
        values = groups[arm]
    ...
    fields = ("job_key", "pair_key", "task_id", "topology", "condition", "arm", "repeat_index", "exposure_count", "outcome_class",
...
async def execute_jobs(tasks, jobs, output, digest, client, base_url, *, max_jobs=None,
    ...
    for job in pending:
        source = None
        if job.get("diagnostic") and job["condition"] != "clean":
            clean_rows = [r for r in state["rows"] if r["condition"] == "clean"]
            if len(clean_rows) != 18 or not all(r.get("final_task_success") is True for r in clean_rows):
                return write_reports(output, jobs, state, ["diagnostic_clean_gate_not_passed"])
        ...
        for number in range(state["attempts"][job["job_key"]] + 1, MAX_ATTEMPTS + 1):
            stop = guard_paid_work(output, digest, phase="job")
            if stop is not None:
                return write_reports(output, jobs, state, blocked, scheduler_stop=stop)
            ...
            try:
                ...
                if job.get("diagnostic"):
                    ...
                    selected_trial = run_diagnostic
                result = await selected_trial(copy.deepcopy(task_map[job["task_id"]]),
                ...
                result = {**result, **start, **completed_usage(client, before, result),
                ...
                error = {**start, **usage_since(client, before, complete=False),
                         "offpeak_blocked": is_peak_exception(exc),
                ...
                if not isinstance(exc, Exception):
                    raise
                if is_peak_exception(exc):
                    stop = record_peak_stop(output, digest, phase="request", start=start)
                    return write_reports(output, jobs, state, blocked, scheduler_stop=stop)
                if state["consecutive_error_attempts"] >= MAX_CONSECUTIVE_ERRORS:
...
def preflight_client(settings):
    if inference_settings() != settings or not os.environ.get("LLM_API_KEY"):
        raise ValueError("explicit Flash inference settings and LLM_API_KEY required")
    client = get_llm_client()
    info = client.model_info
    if (info.provider != PROVIDER or info.model != MODEL or info.client_type != "http_openai_compatible"
            or openai_api_base(validate_deepseek_url(info.base_url)) != settings["api_base_url"]):
        raise ValueError("client does not match the frozen real Flash model")
    started = time.time()
    ...
    with local_inference_transport(), urllib.request.urlopen(request, timeout=10) as response:
        served = json.load(response)
    if MODEL not in {item.get("id") for item in served.get("data", []) if isinstance(item, dict)}:
        raise ValueError("required model is absent from the official model listing")
...
def main(argv=None):
    ...
    try:
        ...
        workflow_lock = locked_workflow(wait=True) if getattr(args, "wait_workflow", False) else locked_workflow()
        with workflow_lock, locked_output(args.output_dir), local_inference_transport():
            ...
            stop = guard_paid_work(args.output_dir, digest, phase="startup")
            if stop is not None:
                write_reports(args.output_dir, config["jobs"], state, scheduler_stop=stop)
                return 1
        ...
        if (summary["circuit_breaker"]["tripped"] or summary["exhausted_job_keys"]
                or summary["source_blocked_job_keys"] or summary.get("scheduler_stop") is not None):
            return 1
