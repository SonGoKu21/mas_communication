...
class Auditor:
    ...
    def manifest(self):
        ...
        if not isinstance(settings, dict):
            ...
            try:
                endpoint = urlsplit(settings.get("api_base_url", ""))
                local = endpoint.scheme in {"http", "https"} and endpoint.hostname in {"localhost", "127.0.0.1", "::1"}
            except ValueError:
                local = False
            if not local:
                self.issue("nonlocal_inference_endpoint")
    ...
    def usage(self, row):
        ...
        if log is not None:
            if not isinstance(log, list) or len(log) != len(requests) or any(
                not isinstance(a, dict) or any(a.get(k) != b.get(k) for k in b) for a, b in zip(log, requests)):
    ...
    def audit(self):
        ...
        for row in rows:
            ...
            try:
                ...
                evaluated = {}
            cases.append({**{k: row.get(k) for k in (*JOB_FIELDS, "run_id", "attempt_id")},
            ...
            attempt_cases.append({**{k: row.get(k) for k in (*JOB_FIELDS, "run_id", "attempt_id", "attempt")},
                                  "terminal_kind": "completed", **usage})
        for row in errors:
            self.privacy(row, row)
            usage = self.usage(row)
            usages.append(usage)
            attempt_cases.append({**{k: row.get(k) for k in (*JOB_FIELDS, "run_id", "attempt_id", "attempt", "status", "error_type")},
        ...
        attempt_cases.extend({**{k: row.get(k) for k in (*JOB_FIELDS, "attempt_id", "attempt")},
